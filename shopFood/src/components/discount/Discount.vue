<template>
    <div class="pop">
      <Header :title="info">
        <section class="h_back" @click="$router.go(-1)" slot="left">
          <i class="iconfont icon-jiantou2"></i>
        </section>
      </Header>

      <div class="main" v-show="!ifEnvelopes">
        <div class="inner">
          <div class="title">
            <span>当前{{info}}</span>
          </div>
          <div class="number">
            <h2>0</h2>
            <span>{{info==="余额"?"元":"积分"}}</span>
          </div>
          <button>{{info==="余额"?"提现":"积分兑换商品"}}</button>
        </div>
      </div>

      <div class="more">
        <h3>最近30天{{info}}记录</h3>
        <div>
          <img src="data:image/png;base64," alt="">
          <p>最近30天无记录</p>
          <span>{{info==="红包"?"抱歉暂无红包可用,去App客户端看看？":"快去尝试下新单吧！"}}</span>
          <button v-show="ifEnvelopes" @click="open">go</button>
        </div>
      </div>


      <Toast_r :show="show_app" :close="close" title="下载">
        <section slot="main" class="pop_m">
          <img src="https://cube.elemecdn.com/c/5f/7e11248ffad35e349d689a21d493fpng.png" alt="">
          <div class="info">
            <h3>更多活动，红包尽在客户端</h3>
            <h2>快来扫码下载吧！</h2>
          </div>
        </section>
      </Toast_r>
    </div>
</template>

<script>
  import Header from "../header/Header"
  import Toast_r from "../toast/Toast1"
  export default {
    name: 'Discount',
    components:{
      Header,Toast_r
    },
    data(){
      return{
        show_app:false
      }
    },
    methods:{
      close(){
        this.show_app=false;
      },
      open(){
        this.show_app=true;
      }
    },
    computed:{
      ifEnvelopes(){
         if ( this.info==="红包") {
           return true;
         }else {
           return false;
         }
      }
    },
    props:{
      info:String
    },
  }
</script>

<style scoped lang="less">
  @r:25rem;
  .pop{
    position: absolute;
    top: 0;
    width: 100vw;
    height: 100vh;
    background: #eee;
    z-index: 9;
    .h_back{
      width: 45/@r;
      height: 100%;
      position: absolute;
      i{
        position: absolute;
        top:50%;
        left: 50%;
        transform: translateY(-50%) translateX(-50%);
        font-size: 22/@r;
        color: white;
      }
    }
    .main{
      width: 100%;
      height: 190/@r;
      background: lightcoral;
      overflow: hidden;
      .inner{
        width: 95%;
        height: 170/@r;
        margin: 5/@r  auto;
        background: white;
        border-radius: 15/@r;
      }
      .title{
        padding-left: 8/@r;
        span{
          font-size: 16/@r;
        }
      }
      .number{
        line-height: 80/@r;
        padding-left: 10/@r;
        h2{
          display: inline;
          font-size: 40/@r;
          font-weight: bold;

        }
        span{
          font-size: 16/@r;
        }
      }
      button{
        width: 90%;
        margin: 0 auto;
        background: limegreen;
        height: 40/@r;
        display: block;
        font-size: 16/@r;
        color: white;
        border-radius: 10/@r;
      }
    }
    .more{
      h3{
        font-size: 14/@r;
        padding: 8/@r;
        color: #999;
      }
      div{
        text-align: center;
        img{
          width: 187/@r;
          height: 117/@r;
        }
        p{
          font-size: 18/@r;
          line-height: 36/@r;
          color: #666;
        }
        span{
          font-size: 14/@r;
          color: #999;
        }
        button{
          /*width: ;*/
          /*margin: 0 auto;*/
          background: limegreen;
          //height: 40/@r;
          //display: block;
          font-size: 12/@r;
          color: white;
          padding: 6/@r;
          border-radius: 4/@r;
        }
      }
    }
    .pop_m{
      img{
        width: 200/@r;
        height: 200/@r;
        display: block;
        margin: 30/@r auto;
      }
      .info{
        text-align: center;
        h3{
          font-size: 18/@r;
          line-height: 30/@r;
        }
        h2{
          font-size: 20/@r;
          line-height: 32/@r;
          color: red;
        }
      }
      .ok{
        width: 28%;
        margin: 40/@r auto 0;
        background: lightcoral;
        height: 40/@r;
        text-align: center;
        border-radius: 20/@r;
        span{
          font-weight: bold;
          font-size: 14/@r;
          line-height: 40/@r;
          color: white;
        }
      }
    }
  }
</style>
